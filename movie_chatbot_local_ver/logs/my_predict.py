...
def my_tokenizer(sentence):  # !
    ...
    for m in str1:  # str에서 단어 하나씩 loop
        if '#' in m:  # 만약 단어에 #가 있다면 (키워드라면)
            if (len(m) > 1 and m[m.find(',') + 1] == '#' and m.find(
                    ',') != -1):  # ,#와 같이 특수문자가 2개 같이 있는 경우는 tokenizer에서 붙여서 토큰화하기 때문에 따로
                analysis.append(',')  # 빼준다.
        ...
        if hash_flag:  # hash_flag가 True라면
            ...
            keyword.append(hashtag)  # ! keyword에 이전에 #부분을 뺀부분을 넣어준다. (keyword를 넣어줌)
...
def predict_unseen_data():
    ...
    my_keyword = my_tokenizer(x_raw[0])  # 해시태그(#, hashtag) 뒤에 키워드(keyword) 얻어온거@ list타입임@
